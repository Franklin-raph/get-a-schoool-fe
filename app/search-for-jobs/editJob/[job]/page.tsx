...
export default function Page() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            if(!jobData.title || !jobData.location || !description) {
                ...
                setLoading(true)
                const response = await patch(`/job-posts/${job}/`, { salary_lower_range:jobData.salary_lower_range, salary_upper_range:jobData.salary_upper_range, location:jobData.location, description })
                setMsg('Job updates successfully.');
    ...
    const getJobInfo = async () => {
        try {
            ...
            setMsg('Failed to fetch job info');
            setLoading(false);
            console.error(err);
        }
    }
    ...
    <div>
        ...
        <div className='bg-[#F5F6F7]'>
            <div className='md:py-[4rem] py-[2rem] max-w-[1600px] mx-auto md:px-[4rem] px-[1.2rem]'>
                ...
                <p className='md:text-[15px] text-[12px]'>Home / <span className='text-[#FF0200]'>Edit Job</span></p>
            </div>
        </div>
