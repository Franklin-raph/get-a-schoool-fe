...
export default function Page() {
    ...
    const updatePost = async () => {
        ...
        console.log({title, content, cover_image: cover_image?.id});
        try {
            setIsLoading(true);
            const response = await patch(`/posts/${id}/`, {title, content, cover_image: cover_image?.id})
        ...
        <div className='md:py-[4rem] py-[2rem] max-w-[1600px] mx-auto md:px-[4rem] px-[1.2rem]'>
            <div className='mt-6'>
                ...
                <input 
                    ...
                    value={title} 
                    type="text" 
                    placeholder='Enter a title for your question here' 
                    className='outline-none block border border-[#C2C5E1] h-[48px] rounded-[6px] w-full pl-2 mt-1' 
                />
            </div>
            <div className='mt-5'>
            ...
            {
                isLoading ?
                <button className='bg-[#FF0200] w-full py-[10px] rounded-[5px] text-white mt-3'>Loading...</button>
                :
                <button onClick={updatePost} className='bg-[#FF0200] w-full py-[10px] rounded-[5px] text-white mt-3'>Update Question</button>
            }
        </div>
        <Footer />
    </div>
